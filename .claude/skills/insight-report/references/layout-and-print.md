...
여백 조정은 한두 줄짜리 고아에만 통한다. 판단 요청 박스(`.ask`, 쪽 나눔 금지)처럼 **7줄 넘는 덩어리가
통째로 다음 쪽으로 넘어가고**, 바로 뒤 참고 섹션(`.aside-sec`)이 새 쪽 강제라서 그 쪽의 70%가 비는
경우가 있다. 2026-08호 12쪽이 그랬다.

- 이때는 여백을 30pt 넘게 깎아도 안 붙는다. 대신 **그 호의 화면판**에서 참고 섹션에 선택형 클래스를 붙인다.
  `<section class="aside-sec flow-in" id="aside">` → 인쇄 CSS의 `.aside-sec.flow-in{break-before:auto}`가
  새 쪽 강제를 풀어 박스 뒤에 참고를 이어 붙인다. 화면에는 영향이 없다.
- **공유 규칙(`.aside-sec{break-before:page}`)은 바꾸지 않는다.** 이미 확정·전달된 호(9월호)의 쪽이 다시 흘러가기 때문이다.
  클래스가 없는 호는 그대로다.
- 결과: 2026-08호 17쪽 → 16쪽, 12쪽(박스만 30%)과 14쪽(추적 끝 25%)의 빈자리가 함께 사라짐.
