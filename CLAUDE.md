...
- **등록 LinkedIn 계정의 최근 게시물** — 접근 방법·한계는 아래 "토큰 효율 규칙" 및
  sources.md의 "LinkedIn" 섹션 참조
...
- **LinkedIn(저비용, 필수 — 2026-08-07 사용자 지정)**: sources.md "LinkedIn" 표의 등록 계정을
  매 스캔 도메인 한정 WebSearch(`allowed_domains=["linkedin.com"]`)로 확인한다 — 직접
  fetch는 차단되므로 시도하지 않는다. 날짜가 확인되지 않거나 최근 1~3일이 아니면 쓰지 않고,
  본문을 읽을 수 없으므로 ★최우선으로 승격하지 않는다("커뮤니티 화제"에 한 줄로). 소득 없는
  날이 정상이다 — 접근 방법·한계는 sources.md에 상세히 기록돼 있다.
