...
- **AI 경영·비즈니스 실증 + AI 교육 트렌드(저비용, 필수 — 2026-09-01 사용자 지정)**:
  ...
  발행량이 많으니 제목으로 1차 스크리닝 후 sources.md의 OR조건 스크리닝 규칙(2026-09-02
  추가)을 따른다 — **구체적 기업명·수치·벤치마크·재현 가능한 교육 방식**(A) **또는
  주목도**(B: HN Algolia `points>50`, 아웃렛 인기 목록, 뉴스레터 참여도) 중 하나만
  충족해도 1~2회 WebFetch로 정독한다. 경영 실증 글은 `uses: ax`, 교육 트렌드 글은
  `uses: course` 후보로 우선 검토하고, evidence는 실측 수치면 measured, 서베이·리포트면
  claimed, 주목도만으로 채택했으면 anecdotal로 구분해 페이지화한다(B로 채택 시 본문에
  근거 명시). A·B 둘 다 없는 일반론·리더십 에세이·교육 정책 오피니언만 제외.
