...
월 1회, AI 역량육성 컬리지 담당 임원 대상. **작성·발행은 `insight-report` 스킬을 따른다**
(`.claude/skills/insight-report/`). 절차, 편집 규칙, 문체, 레이아웃, 인쇄 파이프라인이 모두
거기 있다. 이 문서에는 **스캔 때 필요한 것만** 남긴다.

산출물은 넷. `YYYY-MM.md`(텍스트 원본) · `YYYY-MM.html`(화면판, 아티팩트로 발행) ·
`YYYY-MM-print.html`과 `YYYY-MM.pdf`(인쇄판, `scripts/build-print.mjs`가 화면판에서 생성 —
직접 고치지 않는다).

**스캔에 걸리는 규칙 하나**: 우리 조직의 주제는 AI 활용 스킬·AI의 업무 적용·AI의 기업 활용이다.
학교·대학의 AI 정책, 학업 부정행위, 시험 방식 변화, edtech 시장 재편은 **주 관심사가 아니다.**
이런 페이지에는 `programs:` 필드를 붙이지 않는다. `programs:`가 리포트 3부의 후보 필터이므로,
