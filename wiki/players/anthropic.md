...
**8/25 — 웰빙 연구 $5M 그랜트 프로그램**: AI가 사용자 웰빙에 미치는 영향을 독립적으로
평가하는 오픈소스 벤치마크 연구에 자금·모델 접근·기술 지원을 제공. 컴패니언십 추구·
정신건강 위기 대응처럼 아직 업계 표준이 없는 영역에 임상심리 전문가 등을 끌어들이려는
시도. → [[2026-06-30-claude-science]] (08-27 후속 절 참조)

**8/26 — Claude Cowork에 계정 격리형 내장 브라우저 추가**: 기존 08-12 "Claude in Chrome
사이드패널"(사용자 로그인 세션 공유)과 별개로, 사용자 계정과 완전히 분리된 독립형 브라우저를
Cowork 데스크톱 앱에 신설 — Pro·Max·Team은 이번 주부터, Enterprise는 즉시 사용 가능.
OpenAI가 자체 AI 브라우저 ChatGPT Atlas를 단종(08-09)한 것과 반대로 Anthropic은 브라우저
내장 방향으로 움직이고 있다. → [[2026-08-12-claude-cowork-chrome-integration]] (08-26 후속 절)

**8/27 — 국방부 "공급망 리스크" 블랙리스트 지정, 법원이 위법 판결**: 자율살상무기·대량감시
사용제한을 요구하다 결렬된 $200M 규모 국방부 계약 협상 끝에 Hegseth 국방장관이 Anthropic을
"공급망 리스크"로 지정(사실상 국방부 협력사와의 거래 전면 차단)했던 사건을, Rita Lin
연방판사가 수정헌법 1조(표현의 자유)·5조(적법절차) 위반으로 판단하고 정부에 전면 철회를
명령했다. 08-04 Tino Cuéllar 영입 당시 "누적된 정부 리스크"로만 언급됐던 갈등이 이번
판결로 일단락되는 국면이다. → [[2026-08-27-anthropic-pentagon-blacklist-ruling]]

**8/27 — Model Hardware Standard(MHS) 연구 프리뷰**: AI 에이전트가 실험실·제조 현장의
물리 장비(현미경·액체 취급 장치·로봇팔 등)를 안전하게 직접 조작할 수 있도록 하는 새
표준을 공개. MCP 기반 모델 비종속 설계이며, AWS·Automata·Tecan·Universal Robots가
출시와 동시에 지원 발표. 프리뷰 이후 오픈소스화 예정 — Claude Code(소프트웨어)에 이어
물리 세계로 에이전트 영역을 처음 확장한 시도. → [[2026-08-27-anthropic-model-hardware-standard]]

**8/27 — 과학자 1만 명에게 Claude 무료·할인 제공**: 학술·비영리 연구기관 PI급 대상으로
스탠다드 좌석 무료, 프리미엄 좌석 월 $15인 신설 팀 플랜을 열어 Claude Science 생태계를
확장. 초과 사용량은 프로젝트당 최대 $50,000 크레딧 지원 "AI for Science" 프로그램으로
연결. → [[2026-06-30-claude-science]] (08-27 후속 절)

**8/27 — AI 사이버 방어 공동서한에 서명**: OpenAI 주도로 Google·Microsoft·AWS 등 116개
이상의 기업·기관이 서명한 공개서한에 Anthropic도 이름을 올렸다. AI 기반 사이버공격이
정교해지기 전 "제한된 시간의 창" 안에 방어를 강화하자는 취지 — 8/21
...
**9/12 — Dario Amodei, "AI 스웜이 인터넷을 장악할 수 있다" 경고 + 3단계 감속 계획**:
개인 에세이로 "6~12개월 내 자율 에이전트 스웜이 전체 인터넷을 장악할 수 있다"고 경고하며,
외부 안전 평가자 상시 접근권 부여(1단계) → 업계 공통 안전기준 조율(2단계) → 재귀적
자기개선 속도 국제 제한(3단계)의 실행 계획을 처음으로 회사 차원에서 구체화했다. 07-28
"Pacing the Frontier" 서한이 정부에 대한 청원이었다면, 이번엔 Anthropic 자체 실행
공약이라는 점에서 진전이다. Sam Altman이 몇 시간 뒤 동의, OpenAI도 외부 평가자 상시
배치를 약속. → [[2026-09-12-anthropic-dario-amodei-ai-slowdown-plan]]

**9/13 — Nasdaq 상장 확정 보도, IPO 후속**: Business Insider·Bloomberg가 나란히
Anthropic이 상장 거래소로 Nasdaq을 확정했다고 보도 — 이르면 10월 상장, SpaceX 2026-06
IPO와 맞먹거나 능가하는 규모 목표, 연환산 매출 $65B/년 돌파(작년 말 대비 7배 페이스).
"AI 감속"을 하루 전 공개 촉구한 CEO의 회사가 역대급 몸값 상장을 동시에 추진하는
대비가 눈에 띈다. → [[2026-07-15-anthropic-ipo-investor-meetings]] (09-13 후속 절)

**9/14 — 백악관·David Sacks, Amodei의 AI 감속 계획 거부**: Trump 대통령이 "AI에서
이기는 쪽이 이긴다"며 감속 요구를 일축, 대통령 AI 정책 총괄 David Sacks는 두 회사의
"업계 공통 기준" 요구를 사실상 **규제 포획**이라고 정면 비판("감속하고 싶으면 너희끼리
하면 된다"). 09-12 감속 선언 이틀 만에 정부 차원의 명시적 반박에 부딪힌 것 →
...
**9/25 — Pentagon 블랙리스트 항소심 반전, Akamai $11.6B 클라우드 계약, 창업자
7인 의결권 50.1% 확보 추진**: 세 가지가 같은 날 겹쳤다. ① 연방 항소법원(D.C.
순회구)이 2-1 판결로 국방부의 "공급망 리스크" 지정을 정당하다고 인정 —
08-27 지방법원의 "위법" 판결을 사실상 뒤집었다 (⚠️ 상충 정리는
[[2026-08-27-anthropic-pentagon-blacklist-ruling]] 09-25 후속 절 참고).
② Akamai와 7년 $11.6B 규모 CPU 워크로드 클라우드 계약 체결, 지분 워런트 최대 5%
포함 — 1년이 채 안 되는 기간에 컴퓨트 지출 약정 총액이 $500B를 돌파했다 →
[[2026-09-25-anthropic-akamai-cloud-deal]]. ③ TechCrunch 보도에 따르면 IPO(10월
목표)를 앞두고 7명의 공동창업자가 대부분의 안건에서 결합 50.1% 의결권을 갖는
지배구조를 주주들에게 승인 요청 중 — 상장 후에도 창업자 그룹이 사실상 통제권을
